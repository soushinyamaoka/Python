...
def learning(fileName, degree):
    # パスを取得
    if setting.LOCAL＿MODE:
        modelPath = setting.LOCAL_TSLITE_MODEL_PATH + setting.TSLITE_MODEL_NAME
    else:
        modelPath = setting.TSLITE_MODEL_PATH + setting.TSLITE_MODEL_NAME
    filePath = setting.IMAGE_FILE_PATH + "/"
    modelDir = os.path.join(os.getcwd(), modelPath)
    imagePath1 = filePath + fileName

    # アップロードされた画像から推論
    image = Image.open(imagePath1)
    model = TFLiteModel(modelDir)
    model.load()
    ...
    for pr in predictions:
        conf = float(str(pr["confidence"])[:3])
        if (abs(1 - conf)) < degree :
            nameList.append(pr["label"])
    return nameList
